...
def init_params(options, dim, sentimentDim, isTest):
    """
    ...
    #                           options['dim_proj'])
    params['Wemb'] = numpy.loadtxt('D:/thesis_experiment/cue corpora/lstm/data/'+category+'/'+isTest+type+'_'+category+'_dict_'+str(dim-sentimentDim)+'.txt', delimiter=' ', dtype='float32')
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        ...
        array_preds = numpy.asarray(preds)
        array_targets = numpy.asarray(targets)

        TP += len([product for product in (array_preds & array_targets) if product == 1])
        precision_denominator += len([p for p in array_preds if p == 1])
        recall_denominator += len([t for t in array_targets if t == 1])

    precision = (TP + 0.000000001) / (precision_denominator + 0.000000001)
    recall = (TP + 0.000000001) / (recall_denominator + 0.000000001)

    F_score = (2 * precision * recall) / (precision + recall)

    valid_err = 1. - numpy_floatX(F_score)
...
def train_lstm(
    ...
    # Model options
    embedName = 'D:/thesis_experiment/cue corpora/lstm/result/'+category+'/'+isTest+type+'_'+str(dim_proj)+'_'+str(alpha)
    ...
    params = init_params(model_options, dim_proj, sentimentDim, isTest)
...
def preprocess(type, category, dimension, train_Num, test_Num, isTest):
    import cPickle
    print("preprocessing...")
    train_data_x_zheng = []
    train_data_x_ni = []
    ...
    test_data_x_ni = []
    test_data_y = []
    train_vec_file1 = open('D:/thesis_experiment/cue corpora/lstm/data/'+category+'/'+isTest+type+'_train_'+category+'_en_'+str(dimension)+'.txt', 'r')
    train_vec_file2 = open('D:/thesis_experiment/cue corpora/lstm/data/'+category+'/'+isTest+type+'_train_'+category+'_cn_'+str(dimension)+'.txt', 'r')
    train_label_file = open('D:/thesis_experiment/cue corpora/cue-level/'+isTest+'filter_'+category+'_train_label.txt', 'r')
    ...
    test_vec_file1 = open('D:/thesis_experiment/cue corpora/lstm/data/'+category+'/'+type+'_test_'+category+'_en_'+str(dimension)+'.txt', 'r')
    test_vec_file2 = open('D:/thesis_experiment/cue corpora/lstm/data/'+category+'/'+type+'_test_'+category+'_cn_'+str(dimension)+'.txt', 'r')
    test_label_file = open('D:/thesis_experiment/cue corpora/cue-level/'+category+'_test_label.txt', 'r')
    ...
    output_file = open(category+'.pkl', 'w')
...
if __name__ == '__main__':
    ...
    train_Num = 6230    #TEST_abstract:6230 TEST_fullpaper:1533


    test_Num = 13452    #abstract:13452 fullpaper:1115

    n_word = 137278       #semantic: TEST_abstract:26836 TEST_fullpaper:6773
                         #semanticContex: TEST_abstract:137278 TEST_fullpaper:22039

    maxlength = 29  #abstract:29 fullpaper:23
    isTest  = "TEST_"

    preprocess(type, category, dimension-contextDim, train_Num, test_Num, isTest)
